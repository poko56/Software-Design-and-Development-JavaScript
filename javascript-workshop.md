...
<head>
    ...
    <style>
        ...
        button { padding: 10px 15px; cursor: pointer; background-color: #007bff; color: white; border: none; border-radius: 4px; margin: 5px; }
...
<body>
    ...
    <div class="box">
        <h3>ข้อที่ 1: ปุ่ม 3 รูปแบบ</h3>
        <button onclick="alert('ชื่อนักศึกษา: นายใจดี เรียนสนุก')">ปุ่มที่ 1 (ชื่อนักศึกษา)</button>

        <button id="btn-date">ปุ่มที่ 2 (วันที่ปัจจุบัน)</button>

        <button onclick="showTime()">ปุ่มที่ 3 (เวลาปัจจุบัน)</button>
    </div>

    <div class="box">
        <h3>ข้อที่ 2: ดึงข้อมูลจากกล่องข้อความ</h3>
        <input type="text" id="myInput" placeholder="กรอกข้อความที่นี่">
        <button id="btn-display">แสดงผลข้อความ</button>
    </div>
    ...
    <script>
        ...
        document.getElementById('btn-date').onclick = function() {
            const d = new Date();
            alert("วันที่ปัจจุบันคือ: " + d.toLocaleDateString('th-TH'));
        };

        // ปุ่มแสดงข้อความจากช่องกรอก
        document.getElementById('btn-display').onclick = function() {
            // ดึงค่าโดยใช้ ID 'myInput' แล้วใช้ .value
            const text = document.getElementById('myInput').value;
            
            if (text === "") {
                alert("กรุณากรอกข้อความก่อนครับ!");
            } else {
                alert("ข้อความที่คุณกรอกคือ: " + text);
            }
        };
    </script>
...
<head>
    <meta charset="UTF-8">
    <title>บันทึกผลการทดลอง 2.1</title>
</head>
<body>
    <h1>แบบทดสอบที่ 2.1: การประกาศตัวแปร</h1>
    <p>ตรวจสอบผลลัพธ์ได้ที่ Console (กด F12)</p>

    <script>
        // --- ส่วนที่ 1 ---
        const studentId = "6701234567";
        const studentName = "สมชาย เรียนดี";
        let midtermScore = 25;
        let finalScore = 38;
        ...
        console.log("คะแนนรวม:", (midtermScore + finalScore));

        console.log("--- ข้อมูลใน Object ---");
        console.log(studentInfo);
        
        // ทดลองเปลี่ยนค่าคะแนน (let เปลี่ยนได้)
        midtermScore = 30; 
        console.log("คะแนนกลางภาคใหม่:", midtermScore);
    </script>
</body>
</html>]
```
**รูปผลการทดลอง**
![รูปผลการทดลองที่ 2.1](Resalt/Lab2.1.png)
...
<head>
    <meta charset="UTF-8">
    <title>บันทึกผลการทดลอง 2.2</title>
</head>
<body>
    <h1>แบบทดสอบที่ 2.2: การคำนวณพื้นฐาน</h1>
    <p>ผลการคำนวณแสดงอยู่ใน Console (F12)</p>

    <script>
        // --- ข้อที่ 1: คำนวณคะแนนเฉลี่ย ---
        let mathScore = 85;
        let scienceScore = 78;
        let englishScore = 92;
        ...
        console.log("--- 1. คำนวณคะแนนเฉลี่ย ---");
        console.log("คะแนนวิชาคณิตศาสตร์:", mathScore);
        console.log("คะแนนวิชาวิทยาศาสตร์:", scienceScore);
        console.log("คะแนนวิชาภาษาอังกฤษ:", englishScore);
        console.log("คะแนนเฉลี่ยรวมคือ:", average.toFixed(2)); // .toFixed(2) เพื่อแสดงทศนิยม 2 ตำแหน่ง


        // --- ข้อที่ 2: คำนวณราคาสินค้ารวม VAT 7% ---
        const productName = "คีย์บอร์ดไร้สาย";
        let productPrice = 1500;
        const VAT_RATE = 0.07;

        let totalVat = productPrice * VAT_RATE;
        let totalPrice = productPrice + totalVat;

        console.log("--- 2. คำนวณราคาสินค้ารวม VAT ---");
        console.log("ชื่อสินค้า:", productName);
        console.log("ราคาก่อน VAT:", productPrice, "บาท");
        console.log("ภาษีมูลค่าเพิ่ม (7%):", totalVat, "บาท");
        console.log("ราคาสุทธิที่ต้องจ่าย:", totalPrice, "บาท");
    </script>
</body>
</html>]
```
**รูปผลการทดลอง**
![รูปผลการทดลองที่ 2.2](Resalt/Lab2.2.png)
...
<head>
    <meta charset="UTF-8">
    <title>บันทึกผลการทดลอง 2.3 - Control Flow</title>
</head>
<body>
    <h1>แบบทดสอบที่ 2.3: การควบคุมการทำงาน</h1>
    <p>ตรวจสอบผลลัพธ์ที่ Console (กด F12)</p>

    <script>
        // --- 1. ตรวจสอบเลขคู่หรือเลขคี่ ---
        console.log("--- 1. ตรวจสอบเลขคู่/คี่ ---");
        let number = 15; // ลองเปลี่ยนตัวเลขดูได้ครับ
        if (number % 2 === 0) {
        ...
        // --- 2. ตารางสูตรคูณ (for & while loop) ---
        console.log("--- 2. ตารางสูตรคูณ ---");
        // แม่ 2 ใช้ for loop
        console.log("สูตรคูณแม่ 2 (for loop):");
        for (let i = 1; i <= 12; i++) {
            console.log("2 x " + i + " = " + (2 * i));
        }

        // แม่ 3 ใช้ while loop
        console.log("สูตรคูณแม่ 3 (while loop):");
        let j = 1;
        while (j <= 12) {
        ...
        // --- 3. นับถอยหลัง 10 ถึง 1 ---
        console.log("--- 3. นับถอยหลัง ---");
        for (let count = 10; count >= 1; count--) {
            console.log("นับถอยหลัง: " + count);
        }

        // --- 4. ตรวจสอบช่วงวัยตามอายุ ---
        console.log("--- 4. ตรวจสอบช่วงวัย ---");
        let age = 25; // ลองเปลี่ยนค่าอายุที่นี่
        console.log("อายุ: " + age + " ปี");
        if (age >= 0 && age <= 12) {
            console.log("สถานะ: วัยเด็ก");
        } else if (age >= 13 && age <= 19) {
            console.log("สถานะ: วัยรุ่น");
        } else if (age >= 20) {
            console.log("สถานะ: วัยผู้ใหญ่");
        } else {
            console.log("ข้อมูลอายุไม่ถูกต้อง");
        }
    </script>
</body>
</html>]
```
**รูปผลการทดลอง**
![รูปผลการทดลองที่ 2.3](Resalt/Lab2.3.png)
...
<head>
    <meta charset="UTF-8">
    <title>บันทึกผลการทดลอง 2.4.1 - Functions</title>
</head>
<body>
    <h1>แบบทดสอบที่ 2.4.1: การใช้งาน Functions</h1>
    <p>ตรวจสอบผลลัพธ์ที่ Console (กด F12)</p>

    <script>
        // --- 1. Function คำนวณค่า BMI ---
        // สูตร: BMI = weight / (height_m * height_m)
        function calculateBMI(weight, heightCm) {
            let heightM = heightCm / 100; // แปลงเซนติเมตรเป็นเมตร
            let bmi = weight / (heightM * heightM);
            return bmi.toFixed(2); // ส่งค่ากลับเป็นทศนิยม 2 ตำแหน่ง
        }

        console.log("--- 1. คำนวณ BMI ---");
        let myBMI = calculateBMI(65, 170);
        console.log("น้ำหนัก 65kg, ส่วนสูง 170cm | ค่า BMI คือ: " + myBMI);
        ...
        function smartGreet(name, age) {
            let greeting = "";
            if (age < 15) {
                greeting = "สวัสดีจ้ะ หนู " + name;
            } else if (age < 60) {
                greeting = "สวัสดีครับ คุณ " + name;
            } else {
                greeting = "สวัสดีครับ ท่าน " + name;
            }
            console.log(greeting + " (อายุ " + age + " ปี)");
        }

        console.log("--- 2. ทักทายตามอายุ ---");
        smartGreet("น้องแก้ม", 10);
        smartGreet("สมชาย", 30);
        ...
        function checkPassword(password) {
            if (password.length > 8) {
                return true; // รหัสผ่านยาวกว่า 8 ตัว
            } else {
                return false; // รหัสผ่านสั้นเกินไป
            }
        }

        console.log("--- 3. ตรวจสอบรหัสผ่าน ---");
        let pass1 = "123456";
        let pass2 = "securePassword123";
        
        console.log("รหัส '" + pass1 + "' ผ่านเกณฑ์หรือไม่: " + checkPassword(pass1));
        console.log("รหัส '" + pass2 + "' ผ่านเกณฑ์หรือไม่: " + checkPassword(pass2));
...
```
**รูปผลการทดลอง**
![รูปผลการทดลองที่ 2.4.1](Resalt/Lab2.4.1.png)
...
<head>
    <meta charset="UTF-8">
    <title>บันทึกผลการทดลอง 2.4.2 - Arrow Functions</title>
</head>
<body>
    <h1>แบบทดสอบที่ 2.4.2: การใช้งาน Arrow Functions</h1>
    <p>ตรวจสอบผลลัพธ์ที่ Console (กด F12)</p>

    <script>
        // --- 1. Arrow Function คำนวณค่า BMI ---
        // ใช้รูปแบบย่อบรรทัดเดียว
        const calculateBMI = (weight, heightCm) => (weight / ((heightCm / 100) ** 2)).toFixed(2);

        console.log("--- 1. คำนวณ BMI (Arrow Function) ---");
        console.log("น้ำหนัก 70kg, ส่วนสูง 175cm | ค่า BMI คือ: " + calculateBMI(70, 175));


        // --- 2. Arrow Function ทักทายตามอายุ ---
        const smartGreet = (name, age) => {
            let greeting = age < 15 ? `สวัสดีจ้ะหนู ${name}` : `สวัสดีครับคุณ ${name}`;
            console.log(`${greeting} (อายุ ${age} ปี)`);
        };
        ...
        // --- 3. Arrow Function ตรวจสอบความยาวรหัสผ่าน ---
        // เขียนแบบสั้นที่สุด (คืนค่าเป็น true/false ทันที)
        const isPasswordValid = password => password.length > 8;

        console.log("--- 3. ตรวจสอบรหัสผ่าน ---");
        console.log("รหัส 'abc123' ผ่านเกณฑ์ไหม: " + isPasswordValid("abc123"));
        console.log("รหัส 'mySecurePassword' ผ่านเกณฑ์ไหม: " + isPasswordValid("mySecurePassword"));
...
```
**รูปผลการทดลอง**
![รูปผลการทดลองที่ 2.4.2](Resalt/Lab2.4.2.png)
...
<head>
    ...
    <style>
        ...
        .container { max-width: 400px; padding: 20px; border: 1px solid #ddd; border-radius: 10px; }
        ...
        #result { margin-top: 20px; font-weight: bold; font-size: 1.2rem; color: #007bff; }
    </style>
</head>
<body>

    <div class="container">
        <h3>โปรแกรมคำนวณค่า BMI</h3>
        น้ำหนัก (กก.): <input type="number" id="weight" placeholder="เช่น 65">
        ส่วนสูง (ซม.): <input type="number" id="height" placeholder="เช่น 170">
        
        <button id="calcBtn">คำนวณผลลัพธ์</button>
    ...
    <script>
        ...
        const calculateBMI = () => {
            ...
            // ตรวจสอบว่ากรอกตัวเลขครบถ้วนไหม
            if (isNaN(w) || isNaN(h)) {
            ...
            // แสดงผลออกทางหน้าเว็บ (innerHTML)
            resultDiv.innerHTML = `ค่า BMI ของคุณคือ: ${bmi} <br> สภาวะร่างกาย: ${category}`;
        };

        // จัดการ Event เมื่อคลิกปุ่ม
        document.getElementById('calcBtn').onclick = calculateBMI;
    </script>
...
```
**รูปผลการทดลอง**
![รูปผลการทดลองที่ 3.1](Resalt/Lab3.1.png)
...
<head>
    ...
    <style>
        /* ส่วนของ CSS สำหรับปรับแต่งหน้าตา */
        body {
            font-family: 'Sarabun', sans-serif;
            max-width: 500px;
            margin: 40px auto;
            padding: 20px;
            background-color: #f0f4f8; /* สีพื้นหลังโทนสะอาด */
        }
        ...
        .form-group {
            margin-bottom: 15px; /* จัดระยะห่างระหว่างช่องกรอก */
        }
        ...
        input, select {
            ...
            border: 1px solid #ddd;
            border-radius: 6px;
            box-sizing: border-box; /* ป้องกันช่องกรอกล้นขอบ */
            font-size: 14px;
        }
        ...
        button:hover {
            background-color: #3498db; /* เปลี่ยนสีเมื่อเอาเมาส์วาง */
        }
...
<body>
    ...
    <script>
        // เพิ่มลูกเล่นเมื่อกด Submit
        document.getElementById('bookingForm').onsubmit = (e) => {
            e.preventDefault();
            alert('บันทึกข้อมูลการจองเรียบร้อยแล้ว! ขอบคุณที่ใช้บริการครับ');
        };
    </script>
</body>
</html>]
```
**รูปผลการทดลอง**
![รูปผลการทดลองที่ 3.2.2](Resalt/Lab3.2.2.png)
